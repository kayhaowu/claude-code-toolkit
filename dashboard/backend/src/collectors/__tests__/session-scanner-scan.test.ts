...
describe('scanSessions', () => {
  it('uses .status file as authoritative status source', async () => {
    const mockDeps = {
      ...
      readJson: vi.fn().mockImplementation((path: string) => {
        ...
        if (path.endsWith('446521.hb.dat')) {
          return Promise.resolve({ heartbeat_at: 1773129523, mem_kb: 50000, status: 'idle' });
        }
        return Promise.reject(new Error('not found'));
      }),
      readText: vi.fn().mockImplementation((path: string) => {
        if (path.endsWith('446521.status')) {
          return Promise.resolve('working 1773129523');
        }
    ...
    expect(sessions[0].status).toBe('working'); // from .status file, not heartbeat
    expect(sessions[0].tmux.windowName).toBe('test');
  });

  it('falls back to heartbeat status when .status file missing', async () => {
    const mockDeps = {
      listSessionFiles: vi.fn().mockResolvedValue(['446521.json']),
      readJson: vi.fn().mockImplementation((path: string) => {
        if (path.endsWith('446521.json')) {
      ...
      readText: vi.fn().mockRejectedValue(Object.assign(new Error('not found'), { code: 'ENOENT' })),
      isProcessAlive: vi.fn().mockReturnValue(true),
      getTmuxMap: vi.fn().mockResolvedValue(new Map()),
    ...
    const sessions = await scanSessions(mockDeps);
    expect(sessions).toHaveLength(1);
    expect(sessions[0].status).toBe('working'); // from heartbeat fallback
  });
